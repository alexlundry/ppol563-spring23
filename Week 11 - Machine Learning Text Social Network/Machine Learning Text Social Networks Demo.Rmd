...
Next we clean up some of the typos in the data and also consolidate similar responses.  For example, we fix a misspelling of Obama's first name, and then recode variations of the word "woman" into the same word so that all of these items are rolled up into one word in the cloud.  This process is necessarily iterative.  You'll run the word cloud, look at the results, see a misspelling and then add it to the statement, rerun the cloud, see two related words that need to be rolled up together, rerun the word cloud and so on, until you are satisfied.  
