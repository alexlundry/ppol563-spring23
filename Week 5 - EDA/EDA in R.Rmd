...
Let's revisit the facet, but instead of using counts, we can use `..density..` as the y argument in the `aes` function. This let’s use see the values as a relative measurement rather than an absolute count - so that we can better see the patterns in other areas.  We can provide ..density.. as the y argument in `aes`:
